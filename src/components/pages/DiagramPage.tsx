            ...
            <ZoomActionsContextProvider>
                <PageContainer
                    ...
                    className={cn(
                        ...
                        fullscreenState.isFullscreen && "bg-surface p-0 gap-y-0 md:gap-y-2")}>
                    <Concepts
                        className={cn(
    ...
    return (
        <Container
            ...
            <DiagramActions
                ...
                conceptsPanelEnabled={fullscreenState.conceptsPanelEnabled}
                configPanelEnabled={fullscreenState.configPanelEnabled}
                toggleConceptsPanel={() => fullscreenState.setConceptsPanelEnabled((old) => !old)}
                toggleConfigPanel={() => fullscreenState.setConfigPanelEnabled((old) => !old)}
                toggleBothPanels={() => {
                    const newValue = !(fullscreenState.conceptsPanelEnabled && fullscreenState.configPanelEnabled);
                    fullscreenState.setConceptsPanelEnabled(newValue);
                    fullscreenState.setConfigPanelEnabled(newValue);
                }}
...
function Config(props: {
    ...
    const setRect = useDiagramConfigPanelDimensionsStore((state) => state.setRect);
    useDimensionsListener(containerRef, setRect);
